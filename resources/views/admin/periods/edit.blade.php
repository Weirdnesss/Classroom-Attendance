...
<div class="mb-6">
    ...
    <h1 class="text-xl font-semibold mt-1">Edit Period</h1>
</div>
...
<div class="bg-white border border-gray-200 rounded p-6 max-w-lg">
    <form method="POST" action="{{ route('admin.periods.update', $period) }}">
            ...
            <select name="academic_term_id"
                ...
                @foreach($terms as $term)
                    <option value="{{ $term->id }}"
                        {{ old('academic_term_id', $period->academic_term_id) == $term->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="label" value="{{ old('label', $period->label) }}"
                   class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-400">
