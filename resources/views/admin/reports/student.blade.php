...
<div class="bg-white border border-gray-200 rounded p-4 mb-6 max-w-lg">
    <form method="GET" action="{{ route('admin.reports.student') }}" class="flex gap-3">
...
<div class="bg-white border border-gray-200 rounded overflow-hidden mb-6 max-w-lg">
    ...
    <a href="{{ route('admin.reports.student') }}?q={{ urlencode($query) }}&student_id={{ $s->id }}"
...
<div class="flex items-center justify-between mb-6">
    <div>
        <a href="{{ route('admin.reports.index') }}" class="text-sm text-gray-500 hover:underline">← Reports</a>
        <h1 class="text-xl font-semibold mt-1">{{ $student->full_name }}</h1>
        <p class="text-sm text-gray-500 mt-1">
            {{ $student->student_id }} &middot;
            {{ $student->program->code ?? '—' }} Year {{ $student->year_level }}
        </p>
    </div>
    <a href="{{ route('admin.reports.student.export') }}?student_id={{ $student->id }}"
       class="px-4 py-2 bg-gray-900 text-white text-sm rounded hover:bg-gray-700">
        Export CSV
    </a>
</div>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr>
                ...
                <td class="px-4 py-3 text-gray-500">{{ $log->classSession->teacher->full_name }}</td>
                <td class="px-4 py-3">
            ...
            @empty
            <tr>
                <td colspan="6" class="px-4 py-6 text-center text-gray-400">No attendance records yet.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
