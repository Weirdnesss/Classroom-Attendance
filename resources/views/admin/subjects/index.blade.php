...
<div class="bg-white border border-gray-200 rounded overflow-hidden">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr>
                <td class="px-4 py-3 font-mono font-medium">{{ $subject->code }}</td>
                <td class="px-4 py-3">{{ $subject->name }}</td>
                <td class="px-4 py-3 text-gray-500">{{ $subject->department->code ?? '—' }}</td>
                <td class="px-4 py-3 text-gray-500">{{ $subject->units }}</td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3 text-right space-x-2">
                    ...
                    <form method="POST" action="{{ route('admin.subjects.destroy', $subject) }}"
                          class="inline" onsubmit="return confirm('Delete this subject?')">
                        @csrf @method('DELETE')
            ...
            @empty
            <tr>
                <td colspan="6" class="px-4 py-6 text-center text-gray-400">No subjects yet.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
